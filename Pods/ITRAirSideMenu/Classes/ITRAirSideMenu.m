...
- (void)commonInit
{
    ...
    _contentViewTranslateX = 350.0f;
    
    _menuViewRotatingAngle = 30.0f;
    _menuViewTranslateX = 50.0f;
}
...
- (void)viewDidLoad
{
    ...
    if (self.panGestureEnabled) {
        ...
        UITabBarController *tab = (UITabBarController *)_contentViewController;
        UIViewController *rootViewController = tab.childViewControllers[0];
...
- (void)presentMenuViewContainerWithMenuViewController:(UIViewController *)menuViewController
{
    ...
    if (_menuViewRotatingAngle != 0) {
        ...
        sublayerTransform.m34 = 1.0f / -250.0f;
...
- (void)showLeftMenuViewController
{
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        contentTranslateTransform = CATransform3DTranslate(contentTranslateTransform, _contentViewTranslateX - 250, 0, 0);
        ...
        if (_menuViewRotatingAngle != 0) {
            ...
            sublayerTransform1.m34 = 1.0f / -250.0f;
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateChanged) {
        ...
        if(newLocationPoint.x < 0)
            newLocationPoint.x = 250 + newLocationPoint.x;
        
        //track movement
        if((newLocationPoint.x >= 0 && newLocationPoint.x <= 250))
        {
            [self setAnchorPoint:CGPointMake(1.0, 0.5) forView:_contentViewContainer];
            [self setAnchorPoint:CGPointMake(1.0, 0.5) forView:_contentViewController.view];
            
            //calculation of scale, angle & translate for content view based on pan position
            CGFloat contentViewScale = 1 + (_contentViewScaleValue - 1)* (newLocationPoint.x / 250);
            CGFloat angle = (newLocationPoint.x - _lastPoint.x) * _contentViewRotatingAngle/250;
            CGFloat tranformX = newLocationPoint.x * _contentViewTranslateX/250;
            ...
            [UIView animateWithDuration:self.animationDuration animations:^{
                ...
                if (angle != 0) {
                    CGFloat menuAngle = _menuViewRotatingAngle - (newLocationPoint.x) * _menuViewRotatingAngle/250;
                ...
                CGFloat menuTransformValue = (_menuViewTranslateX * newLocationPoint.x/250) - _menuViewTranslateX - _menuViewContainer.frame.origin.x;
